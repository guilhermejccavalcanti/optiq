...
public class JdbcTest {
    ...
    public static final Method GENERATE_STRINGS_METHOD = Types.lookupMethod(JdbcTest.class, "generateStrings", Integer.TYPE);

    public static final Method STRING_UNION_METHOD = Types.lookupMethod(JdbcTest.class, "stringUnion", Queryable.class, Queryable.class);
   ...
   */
    ...
    public void testFunction() throws SQLException, ClassNotFoundException {
        ...
        final TableFunction<Object> tableFunction = Schemas.methodMember(GENERATE_STRINGS_METHOD, typeFactory);
        rootSchema.addTableFunction(new TableFunctionInSchemaImpl(schema, "GenerateStrings", tableFunction));
    ...
    public static Queryable<IntString> generateStrings(final int count) {
        return new BaseQueryable<IntString>(null, IntString.class, null) {
   ...
   * It is empty but it should still have column definitions. */
    ...
    public void testMetaDataPrimaryKeys() throws ClassNotFoundException, SQLException {
        ...
        // catalog never contains primary keys
    ...
    public void testLikeToRegex() {
        ...
        // trailing char fails match
        checkLikeToRegex(false, "abc", "abcd");
        // leading char fails match
        checkLikeToRegex(false, "abc", "0abc");
        // case-sensitive match
        checkLikeToRegex(false, "abc", "aBc");
        // nothing special about brackets
        checkLikeToRegex(true, "a[b]c", "a[b]c");
        // nothing special about dollar
        checkLikeToRegex(true, "a$c", "a$c");
        // nothing special about dollar
        checkLikeToRegex(false, "a$", "a");
   ...
   */
    ...
    public void testSimple() {
        OptiqAssert.assertThat().with(OptiqAssert.Config.FOODMART_CLONE).query("SELECT 1").returns("EXPR$0=1\n");
    }
    ...
    public void testOrderByCase() {
        OptiqAssert.assertThat().with(OptiqAssert.Config.FOODMART_CLONE).query("select \"time_by_day\".\"the_year\" as \"c0\" from \"time_by_day\" as \"time_by_day\" group by \"time_by_day\".\"the_year\" order by CASE WHEN \"time_by_day\".\"the_year\" IS NULL THEN 1 ELSE 0 END, \"time_by_day\".\"the_year\" ASC").returns("c0=1997\n" + "c0=1998\n");
    }
   ...
   * {@link #testExplainJoin()}. */
    ...
    public void testExplainJoinOrderingWithOr() {
        OptiqAssert.assertThat().with(OptiqAssert.Config.FOODMART_CLONE).query(FOODMART_QUERIES.get(47).left).explainContains("xxx");
    }
   ...
   * internally. */
    ...
    public void testNullableTimestamp() {
        OptiqAssert.assertThat().with(OptiqAssert.Config.FOODMART_CLONE).query("select \"hire_date\" from \"employee\" where \"employee_id\" = 1").returns("hire_date=1994-12-01 00:00:00\n");
    }

    @Test
    public void testValues() {
        OptiqAssert.assertThat().query("values (1), (2)").returns("EXPR$0=1\n" + "EXPR$0=2\n");
    }

    @Test
    public void testValuesAlias() {
        OptiqAssert.assertThat().query("select \"desc\" from (VALUES ROW(1, 'SameName')) AS \"t\" (\"id\", \"desc\")").returns("desc=SameName\n");
    }

    @Test
    public void testValuesMinus() {
        OptiqAssert.assertThat().query("values (-2-1)").returns("EXPR$0=-3\n");
    }
   ...
   */
    ...
    public void testValuesComposite() {
        OptiqAssert.assertThat().query("values (1, 'a'), (2, 'abc')").returns("EXPR$0=1; EXPR$1=a  \n" + "EXPR$0=2; EXPR$1=abc\n");
    }

    @Test
    public void testInnerJoinValues() {
        OptiqAssert.assertThat().with(OptiqAssert.Config.LINGUAL).query("select empno, desc from sales.emps,\n" + "  (SELECT * FROM (VALUES (10, 'SameName')) AS t (id, desc)) as sn\n" + "where emps.deptno = sn.id and sn.desc = 'SameName' group by empno, desc").returns("EMPNO=1; DESC=SameName\n");
    }

    @Test
    public void testDistinctCount() {
        final String s = "select \"time_by_day\".\"the_year\" as \"c0\", sum(\"sales_fact_1997\".\"unit_sales\") as \"m0\" from \"time_by_day\" as \"time_by_day\", \"sales_fact_1997\" as \"sales_fact_1997\" where \"sales_fact_1997\".\"time_id\" = \"time_by_day\".\"time_id\" and \"time_by_day\".\"the_year\" = 1997 group by \"time_by_day\".\"the_year\"";
        OptiqAssert.assertThat().with(OptiqAssert.Config.FOODMART_CLONE).query(s).returns("c0=1997; m0=266773.0000\n");
    }
   ...
   * the column allows nulls. */
    ...
    private OptiqAssert.AssertQuery predicate(String foo) {
        return OptiqAssert.assertThat().with(OptiqAssert.Config.REGULAR).query("select * from \"hr\".\"emps\"\n" + "where " + foo).runs();
    }

    /** Tests the TABLES table in the information schema. */
    @Test
    public void testMetaTables() {
        OptiqAssert.assertThat().with(OptiqAssert.Config.REGULAR_PLUS_METADATA).query("select * from \"metadata\".TABLES").returns(OptiqAssert.checkResultContains("tableSchem=metadata; tableName=COLUMNS; tableType=SYSTEM_TABLE; "));
        OptiqAssert.assertThat().with(OptiqAssert.Config.REGULAR_PLUS_METADATA).query("select count(distinct \"tableSchem\") as c\n" + "from \"metadata\".TABLES").returns("C=3\n");
    }

    /** Tests that {@link java.sql.Statement#setMaxRows(int)} is honored. */
    @Test
    public void testSetMaxRows() throws Exception {
        OptiqAssert.assertThat().with(OptiqAssert.Config.REGULAR).doWithConnection(new Function1<OptiqConnection, Object>() {
   ...
   * a JDBC database). */
    ...
    public void testModel() {
        OptiqAssert.assertThat().withModel(FOODMART_MODEL).query("select count(*) as c from \"foodmart\".\"time_by_day\"").returns("C=730\n");
    }
   ...
   * schema. */
    ...
    public void testModelCustomSchema() throws Exception {
        final OptiqAssert.AssertThat that = OptiqAssert.assertThat().withModel("{\n" + "  version: '1.0',\n" + "  defaultSchema: 'adhoc',\n" + "  schemas: [\n" + "    {\n" + "      name: 'empty'\n" + "    },\n" + "    {\n" + "      name: 'adhoc',\n" + "      type: 'custom',\n" + "      factory: '" + MySchemaFactory.class.getName() + "',\n" + "      operand: {'tableName': 'ELVIS'}\n" + "    }\n" + "  ]\n" + "}");
    ...
    public void testModelImmutableSchemaCannotContainView() throws Exception {
        final OptiqAssert.AssertThat that = OptiqAssert.assertThat().withModel("{\n" + "  version: '1.0',\n" + "  defaultSchema: 'adhoc',\n" + "  schemas: [\n" + "    {\n" + "      name: 'empty'\n" + "    },\n" + "    {\n" + "      name: 'adhoc',\n" + "      type: 'custom',\n" + "      tables: [\n" + "        {\n" + "          name: 'v',\n" + "          type: 'view',\n" + "          sql: 'values (1)'\n" + "        }\n" + "      ],\n" + "      factory: '" + MySchemaFactory.class.getName() + "',\n" + "      operand: {\n" + "           'tableName': 'ELVIS',\n" + "           'mutable': false\n" + "      }\n" + "    }\n" + "  ]\n" + "}");
        that.connectThrows("Cannot define view; parent schema " + "DelegatingSchema(delegate=ReflectiveSchema(target=HrSchema)) is " + "not mutable");
    }
   ...
   * {@link net.hydromatic.optiq.jdbc.Handler.ResultSink}. */
    ...
    public void testAutomaticTemporaryTable() throws Exception {
        ...
        OptiqAssert.assertThat().with(new OptiqAssert.ConnectionFactory() {
    ...
    public void testExplain() {
        final OptiqAssert.AssertThat with = OptiqAssert.assertThat().with(OptiqAssert.Config.FOODMART_CLONE);
   ...
   * is executed. */
    ...
    public void testCurrentTimestamp() throws Exception {
        ...
        }).doWithConnection(new Function1<OptiqConnection, Void>() {
    ...
    private void checkGetTimestamp(Connection con) throws SQLException {
        ...
        // +0300 always
        TimeZone GMT03 = TimeZone.getTimeZone("GMT+03");
        // -0500 always
        TimeZone GMT05 = TimeZone.getTimeZone("GMT-05");
        // +1000 always
        TimeZone GMT13 = TimeZone.getTimeZone("GMT+13");
        ...
        int c = 1;
        // timestamp: 1970-01-01 00:00:00
        // Convert timestamp to +0100
        ts = rs.getTimestamp(c);
        // 1970-01-01 00:00:00 +0100
        assertEquals(-3600000L, ts.getTime());
        // Convert timestamp to UTC
        ts = rs.getTimestamp(c, cUTC);
        // 1970-01-01 00:00:00 +0000
        assertEquals(0L, ts.getTime());
        // Convert timestamp to +0300
        ts = rs.getTimestamp(c, cGMT03);
        // 1970-01-01 00:00:00 +0300
        assertEquals(-10800000L, ts.getTime());
        // Convert timestamp to -0500
        ts = rs.getTimestamp(c, cGMT05);
        // 1970-01-01 00:00:00 -0500
        assertEquals(18000000L, ts.getTime());
        // Convert timestamp to +1300
        ts = rs.getTimestamp(c, cGMT13);
        // 1970-01-01 00:00:00 +1300
        assertEquals(-46800000, ts.getTime());
        s = rs.getString(c);
        ...
        if (false) {
            ...
            assertEquals(1104580800000L, ts.getTime());
            // TZ irrelevant, as above
            ts = rs.getTimestamp(c, cUTC);
            // 2005-01-01 12:00:00 UTC
            assertEquals(1104580800000L, ts.getTime());
            // TZ irrelevant, as above
        ...
        // Convert timestamp to +0100
        ts = rs.getTimestamp(c);
        // 2005-01-01 15:00:00 +0100
        assertEquals(1104588000000L, ts.getTime());
        // Convert timestamp to UTC
        ts = rs.getTimestamp(c, cUTC);
        // 2005-01-01 15:00:00 +0000
        assertEquals(1104591600000L, ts.getTime());
        // Convert timestamp to +0300
        ts = rs.getTimestamp(c, cGMT03);
        ...
        // Convert timestamp to -0500
        ts = rs.getTimestamp(c, cGMT05);
        // 2005-01-01 15:00:00 -0500
        assertEquals(1104609600000L, ts.getTime());
        // Convert timestamp to +1300
        ts = rs.getTimestamp(c, cGMT13);
        // 2005-01-01 15:00:00 +1300
        assertEquals(1104544800000L, ts.getTime());
        s = rs.getString(c);
        assertEquals("2005-01-01 15:00:00", s);
        ++c;
        // time: 15:00:00
        ts = rs.getTimestamp(c);
        // 1970-01-01 15:00:00 +0100
        assertEquals(50400000L, ts.getTime());
        ts = rs.getTimestamp(c, cUTC);
        // 1970-01-01 15:00:00 +0000
        assertEquals(54000000L, ts.getTime());
        ts = rs.getTimestamp(c, cGMT03);
        // 1970-01-01 15:00:00 +0300
        assertEquals(43200000L, ts.getTime());
        ts = rs.getTimestamp(c, cGMT05);
        // 1970-01-01 15:00:00 -0500
        assertEquals(72000000L, ts.getTime());
        ts = rs.getTimestamp(c, cGMT13);
        // 1970-01-01 15:00:00 +1300
        assertEquals(7200000L, ts.getTime());
        s = rs.getString(c);
        ...
        // date: 2005-01-01
        ts = rs.getTimestamp(c);
        // 2005-01-01 00:00:00 +0100
        assertEquals(1104534000000L, ts.getTime());
        ts = rs.getTimestamp(c, cUTC);
        // 2005-01-01 00:00:00 +0000
        assertEquals(1104537600000L, ts.getTime());
        ts = rs.getTimestamp(c, cGMT03);
        // 2005-01-01 00:00:00 +0300
        assertEquals(1104526800000L, ts.getTime());
        ts = rs.getTimestamp(c, cGMT05);
        // 2005-01-01 00:00:00 -0500
        assertEquals(1104555600000L, ts.getTime());
        ts = rs.getTimestamp(c, cGMT13);
        // 2005-01-01 00:00:00 +1300
        assertEquals(1104490800000L, ts.getTime());
        s = rs.getString(c);
        // 2005-01-01 00:00:00 +0100
        assertEquals("2005-01-01", s);
        ++c;
        assertTrue(!rs.next());
    }
    ...
    public static class HrSchema {
        ...
        public final Employee[] emps = { new Employee(100, 10, "Bill", 10000, 1000), new Employee(200, 20, "Eric", 8000, 500), new Employee(150, 10, "Sebastian", 7000, null), new Employee(110, 10, "Theodore", 11500, 250) };

        public final Department[] depts = { new Department(10, "Sales", Arrays.asList(emps[0], emps[2])), new Department(30, "Marketing", Collections.<Employee>emptyList()), new Department(40, "HR", Collections.singletonList(emps[1])) };
    }
    ...
    public static class FoodmartJdbcSchema extends JdbcSchema {
        ...
        public FoodmartJdbcSchema(MutableSchema parentSchema, DataSource dataSource, SqlDialect dialect, String catalog, String schema, JavaTypeFactory typeFactory, Expression expression) {
            super(parentSchema.getQueryProvider(), parentSchema, "FoodMart", dataSource, dialect, catalog, schema, typeFactory, expression);
        }

        public final Table<Customer> customer = getTable("customer", Customer.class);
    }
    ...
    public abstract static class AbstractModifiableTable<T> extends AbstractTable<T> implements ModifiableTable<T> {

        protected AbstractModifiableTable(Schema schema, Type elementType, RelDataType relDataType, String tableName) {
    ...
    public static class EmpDeptTableFactory implements TableFactory<Table> {
        ...
        public Table create(Schema schema, String name, Map<String, Object> operand, RelDataType rowType) {
            final Class clazz;
            final Object[] array;
            ...
            return new AbstractTable(schema, clazz, schema.getTypeFactory().createType(clazz), name) {
    ...
    public static class MySchemaFactory implements SchemaFactory {
        ...
        public Schema create(MutableSchema parentSchema, String name, Map<String, Object> operand) {
            final ReflectiveSchema schema = ReflectiveSchema.create(parentSchema, name, new HrSchema());
   ...
   * a temporary table. */
    public static class AutoTempDriver extends net.hydromatic.optiq.jdbc.Driver {
        ...
        protected Handler createHandler() {
            return new HandlerImpl() {
                ...
                @Override
                public void onStatementExecute(OptiqStatement statement, ResultSink resultSink) {
    ...
    public static class HandlerDriver extends net.hydromatic.optiq.jdbc.Driver {
        ...
        private static final ThreadLocal<Handler> HANDLERS = new ThreadLocal<Handler>();
